"""TikTok 크롤러 -> fast_crawl save_to_db -> DB 저장 테스트."""
...
async def main():
    ...
    async with async_session() as session:
        before_details = (await session.execute(
            select(func.count(AdDetail.id)).where(AdDetail.ad_type == "tiktok_creative_center")
        )).scalar() or 0
        before_snaps = (await session.execute(
            select(func.count(AdSnapshot.id)).where(AdSnapshot.channel == "tiktok_ads")
        )).scalar() or 0
    print(f"Before: {before_details} details, {before_snaps} snapshots")

    # 크롤링
    print("Crawling TikTok Creative Center...")
    async with TikTokAdsCrawler() as crawler:
        result = await crawler.crawl_keyword("", PERSONAS["M30"], PC_DEVICE)
    ...
    await save_to_db("tiktok_ads", result, "", "M30", "pc")

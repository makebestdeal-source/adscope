...
AdScope 과금은 고정 월정액 단독 구조보다 **기본 구독 + 사용량 크레딧 + Enterprise 별도 계약**이 적합하다.

| 플랜 | 대상 | 기본 과금 | 사용량 과금 |
|------|------|----------|-------------|
| Trial | 검토 고객 | 제한 무료 체험 | 조회, 리포트, export 한도 제한 |
...
### 10.2 Enterprise 계약 원칙

- 일반 플랜과 분리해 좌석/워크스페이스 접근 비용과 데이터/분석 사용량을 따로 산정한다.
- 연간 최소 계약금 또는 선불 크레딧 풀을 두고, 초과분은 월별 정산한다.
- SSO, 권한관리, 감사로그, 전용 리포트, 커스텀 광고주 풀, SLA는 Enterprise 전용으로 둔다.
- 월별 예산 상한, 사용자별 사용량 제한, 사용량 알림을 관리 기능에 포함한다.
